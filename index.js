...
const start = async (tobz = new Client()) => {
        console.log('------------------------------------------------')
        lolcatjs.fromString(color(figlet.textSync('IKKYH BOT', { horizontalLayout: 'full' })))
        console.log('------------------------------------------------')
        ...
        tobz.onAddedToGroup(async (chat) => {
            if(isWhite(chat.id)) return tobz.sendText(chat.id, `Helo ges Its me Ikkyh Bot, Ketik ${prefix}help Untuk Melihat List Command Bot...`)
            if(mtcState === false){
                const groups = await tobz.getAllGroups()
                // BOT group count less than
                if(groups.length > groupLimit){
                    await tobz.sendText(chat.id, 'Sori coy, gwe udah gabung kebanyakan grup. chatt gwe di wa.me/628886600839 kalau mau masukin bot').then(async () =>{
                        tobz.deleteChat(chat.id)
                        tobz.leaveGroup(chat.id)
                    })
                }else{
                    if(chat.groupMetadata.participants.length < memberLimit){
                        await tobz.sendText(chat.id, `Sori coy, BOT keluar kalo member group tidak melebihi ${memberLimit} orang`).then(async () =>{
                            tobz.deleteChat(chat.id)
                            tobz.leaveGroup(chat.id)
                        })
                    }else{
                        if(!chat.isReadOnly) tobz.sendText(chat.id, 'Helo ges Its me Ikkyh Bot, Ketik ${prefix}help Untuk Melihat List Command Bot...')
                    }
                }
            }else{
                await tobz.sendText(chat.id, 'Ikkyh Bot sedang maintenance, coba lain hari').then(async () => {
                    tobz.deleteChat(chat.id)
                    tobz.leaveGroup(chat.id)
                })
            }
        ...
        tobz.onIncomingCall(( async (call) => {
            await tobz.sendText(call.peerJid, 'Jangan nelpon njing. nelfon = block!.\nJika ingin membuka block harap chat Owner!')
