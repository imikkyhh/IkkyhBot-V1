...
Jika ingin melihat list orang random mencari jodoh ketik ${prefix}cjcowo`)
 ...
 //====================================================Sampe Sini Aja===========================================
        case prefix+'groupinfo' :
            ...
            var stprt = _antistiker.includes(chat.id)
...
🌐️ *URL*: ${url}`
	...
	case prefix+'bitly':
        ...
        case prefix+'nhentai': // SEARCH NHENTAI
			...
			if (!isPremium) return tobz.reply(from, `❗️ *Khusus Premium* ❗️\nUntuk membeli user Premium hubungi wa.me/628886600839.\nPremium 1 bulan : 10k\nJadi Admin+Premium : 15k\n\nUntuk fittur admin bisa dicek dengan ketik ${prefix}adminmenu`, id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/628886600839', id)
          break
        ...
        case prefix+'video': // SEARCH VIDEO FROM YOUTUBE
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'Dalam Perbaikan!!', id)
            break
        ...
        case prefix+'xnxx':
            ...
            try {
                tobz.reply(from, mess.wait, id)
                const resq = await axios.get('https://api.vhtear.com/xxxdownload?link=args[1]&apiKey=gxv277dt11')
...
• Limit: ${limitCount-updated.limit}`
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Ikkyh!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin Ikkyh!`, id)
            ...
            break
            case prefix+'premium':
                if (!isOwner) return await tobz.reply(from, 'Perintah ini hanya untuk Owner Ikkyh', id)
                if (args1.length == 0) return await tobz.reply(from, 'format salah', id)
                if (ar[1] === 'add') {
                    if (mentionedJidList.length == 1) {
                        for (let benet of mentionedJidList) {
                            if (benet === botNumber) return await tobz.reply(from, 'format salah ', id)
                            premium.addPremiumUser(benet, args1[2], _premium)
                            await tobz.reply(from, `*「 PREMIUM ADDED 」*\n\n➸ *ID* : ${benet.replace(/@c.us/g,'')}\n➸ *Expired* : ${ms(toMs(args1[2])).days} day(s) ${ms(toMs(args1[2])).hours} hour(s) ${ms(toMs(args1[2])).minutes} minute(s)`, id)
                        }
                    } else {
                    ...
                    if (mentionedJidList.length == 0) {
                        if (mentionedJidList[0] === botNumber) return await tobz.reply(from, 'ketik #', id)
                        ...
                        await tobz.reply(from, 'sukses menghapus user premium', id)
                    } else {
                        _premium.splice(premium.getPremiumPosition(args1[1] + '@c.us', _premium), 1)
                        fs.writeFileSync('./lib/database/premium.json', JSON.stringify(_premium))
                        await tobz.reply(from, 'Telah terdaftar ke dalam database', id)
                    }
                } else {
                    await tobz.reply(from, 'pilih add apa del', id)
                }
            break
        case prefix+'unban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Ikkyh!', id)
        ...
        case prefix+'profile':
            ...
            if (isGroupMsg) {
                if (!quotedMsg) {
					...
					var premi = isPremium ? '*♛Premium*' : '*♟Tidak*'
                    if (found !== false) {
                    ...
                    var namae = quotedMsg.sender.name(qmid)
            ...
            case prefix+'cekpremium':
                if(isReg(obj)) return
                if (!isPremium) return await tobz.reply(from, `Belum Premium`, id)
                const cekExp = ms(premium.getPremiumExpired(sender.id, _premium) - Date.now())
                await tobz.reply(from, `「 *PREMIUM EXPIRE* 」\n\n*Nama : ${pushname}*\n*Nomor : ${sender.id.replace('@c.us', '')}*\n*Sisa Premium : ${cekExp.days} hari ${cekExp.hours} jam ${cekExp.minutes} menit*`, id)
            break
